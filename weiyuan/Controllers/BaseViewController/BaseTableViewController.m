...
- (BOOL)requestDidFinish:(BSClient*)sender obj:(NSDictionary*)obj {
    if (isloadByslime && client == sender) {
...
- (void)getCloudUserByID:(NSString *)cloudId{
    ...
    client = [[BSClient alloc] initWithDelegate:self action:@selector(requestCloudDidFinish:obj:)];
    [client subsDetail:cloudId userId:[BSEngine currentUserId]];
}
