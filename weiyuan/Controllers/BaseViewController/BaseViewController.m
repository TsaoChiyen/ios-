...
#pragma mark - Refresh Current User After user info edited

/**
 *	Copyright © 2015 tcy@dreamisland. All rights reserved.
 *
 *	Refresh Current User After user info edited
 *
 */
- (void)refreshCurrentUser {
    BSClient *userclient = [[BSClient alloc] initWithDelegate:self action:@selector(refreshUserDidFinish:obj:)];
    [userclient getUserInfoWithuid:[[BSEngine currentEngine] user].uid];
}

- (BOOL)refreshUserDidFinish:(id)sender obj:(NSDictionary *)obj {
    NSDictionary *dic = [obj getDictionaryForKey:@"data"];
    if (dic.count > 0) {
        User *user = [User objWithJsonDic:dic];
        [user insertDB];
        
        if ([[[BSEngine currentEngine] user].uid isEqualToString:user.uid]) {
