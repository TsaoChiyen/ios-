...
- (void)viewDidLoad {
    ...
    self.navigationItem.title = session.name.length?session.name:session.message.displayName;;
    ...
    if (!_isSearchMode) {
        ...
        actionBar = [[SessionActionBar alloc] initWithOrigin:CGPointMake(0.0f, self.view.height - 44.0f) withMenu:session.typechat==forChatTypeServiceChat];
    ...
    if (session.typechat==forChatTypeServiceChat || session.typechat==forChatTypeSubChat) {
        [self setRightBarButtonImage:LOADIMAGE(@"people_n") highlightedImage:LOADIMAGE(@"people_d") selector:@selector(showDetailCon)];
    }
    ...
    //缓存
    NSString *data = [[NSUserDefaults standardUserDefaults] objectForKey:[NSString stringWithFormat:@"SessionMenu%@",session.uid]];
    if (data) {
        actionBar.menuData = [JSON objectFromJSONString:data];
    }
}
...
- (UIButton *)showDetail{
    ...
    if (nil==_showDetail) {
        _showDetail = [UIButton buttonWithType:UIButtonTypeInfoLight];
        [_showDetail addTarget:self action:@selector(showDetailCon) forControlEvents:UIControlEventTouchUpInside];
...
- (void)showDetailCon{
    [self getCloudUserByID:self.session.uid];
}

- (void)viewWillAppear:(BOOL)animated{
    
    [super viewWillAppear:animated];
    if (session.typechat==forChatTypeServiceChat) {
        //服务号类型 ，加载菜单
        [self loadMenu];
    }
}
...
- (void)loadMenu{
    BSClient * sclient = [[BSClient alloc] initWithDelegate:self action:@selector(requestMenuDidFinish:obj:)];
    [sclient getCloudMenu:session.uid];
}

- (BOOL)requestMenuDidFinish:(BSClient*)sender obj:(NSDictionary*)obj {
    
    if ([obj isKindOfClass:[NSDictionary class]]) {
        actionBar.menuData = [obj getArrayForKey:@"data"];
        
        //缓存
        NSString *data = [JSON stringWithObject:actionBar.menuData];
        if (data) {
            [[NSUserDefaults standardUserDefaults] setObject:data forKey:[NSString stringWithFormat:@"SessionMenu%@",session.uid]];
...
/* 菜单点击 */
- (BOOL)actionBarMenuDidTap:(NSDictionary*)menu{
    ...
    /*
     {
     "imgurl": "http://123.56.101.233:8082/weiyuan/Uploads/Picture/avatar/1/s_5088a6424d9b14c2e00617f190c8c71d.jpg",
     "title": "这个是新闻标题",
    ...
    SessionNewsDetailViewController *newDetail =[[SessionNewsDetailViewController alloc] init];
    NSString *urlStr =menu[@"link"];
    if ([urlStr isEqualToString:@""] || !urlStr) {
        
        NSString *menuid = menu[@"menuid"];
        BSClient *theClient = [[BSClient alloc] initWithDelegate:self action:@selector(reqFunction:obj:)];
        NSMutableDictionary *params = [NSMutableDictionary dictionary];
        if (menuid)[params setObject:menuid forKey:@"menuid"];
        if ([BSEngine currentUserId])  [params setObject:[BSEngine currentUserId] forKey:@"uid"];
        [theClient loadRequestWithDoMain:YES
                         methodName:@"User/Api/getmenufunction"
                             params:params
        ...
        [newDetail setTitle:menu[@"name"] url:[NSURL URLWithString:urlStr]];
        [self.navigationController pushViewController:newDetail animated:YES];
    }
    return YES;
}

- (BOOL)reqFunction:(id)sender obj:(NSDictionary *)obj {
...
- (UITableViewCell *)tableView:(UITableView *)sender cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.newsDidTap = ^(NSDictionary *newsData){
        /*
         newsData
         ...
         */
        //TODO ZYM 跳转到新闻详情
        NSMutableDictionary *newsDic = [NSMutableDictionary dictionaryWithDictionary:newsData];
        
        NSString *urlStr =newsDic[@"url"];
        if(![urlStr hasPrefix:@"http://"])
        {
            urlStr = [@"http://" stringByAppendingString:urlStr];
        }
        [newsDic setObject:urlStr forKey:@"url"];
        
        
        [newsDic setObject:self.session.uid forKey:@"uid"];
        SessionNewsDetailViewController *newDetail =[[SessionNewsDetailViewController alloc] init];
        [newDetail setTitle:newsData[@"title"] url:[NSURL URLWithString:newsDic[@"url"]]];
        [newDetail setNewsDic:newsDic];
        [self.navigationController pushViewController:newDetail animated:YES];
    };
...
- (void)tableView:(UITableView *)sender handleTableviewCellLongPressed:(NSIndexPath *)indexPath {
    ...
    if (msg.typefile == forFileText) {
        // 复制
        sheet = [[CameraActionSheet alloc] initWithActionTitle:nil TextViews:nil CancelTitle:@"取消" withDelegate:self otherButtonTitles:@"复制", @"转发", @"收藏",@"删除", nil];
        sheet.mark = @"forFileText";
    } else {
        sheet = [[CameraActionSheet alloc] initWithActionTitle:nil TextViews:nil CancelTitle:@"取消" withDelegate:self otherButtonTitles:@"转发", @"收藏",@"删除", nil];
        sheet.mark = @"forOther";
    }
...
- (void)cameraActionSheet:(CameraActionSheet *)sender didDismissWithButtonIndex:(NSInteger)buttonIndex {
    ...
    if ([sender.mark isEqualToString:@"forFileText"]) {
        if (buttonIndex == 0) {
            ...
            // 删除消息
            [self deleteMessage:msg forIndexPaths:sender.indexPath];
        }
...
- (void)deleteMessage:(Message *)msg forIndexPaths:(NSIndexPath *)path{
    
    [msg deleteFromDB];
    Message *lastMessage = [contentArr lastObject];
    
    [contentArr removeObject:msg];
    [tableView reloadData];
    
    if (msg == lastMessage && self.session.typechat==forChatTypeSubChat && msg && lastMessage) {
        
        Session *subSession = [Session getSessionWithID:@"9110211"];
        if ([subSession.message.content isEqualToString:msg.content]) {
            Message *newMsg = [[contentArr lastObject] copy];
            newMsg.displayName = [[contentArr lastObject] displayName];
            if (newMsg) {
                //订阅号固定SessionID
                newMsg.uid = @"9110211";
                if (newMsg.isSendByMe) {
                    newMsg.toId = @"9110211";
                }else{
                    newMsg.fromId = @"9110211";
                }
                newMsg.displayName = session.message.displayName;
                //存储msg
                [newMsg insertDB];
                //更新原有会话
                ...
                [subSession.message deleteFromDB];
            }
        }
    }
