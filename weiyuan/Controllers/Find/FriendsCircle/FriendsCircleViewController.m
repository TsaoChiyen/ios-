...
- (UITableViewCell*)tableView:(UITableView *)sender cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    UIImageView * view = VIEWWITHTAG(cell.contentView, 11);
    UILabel * label = VIEWWITHTAG(cell.contentView, 12);
    if (!view) {
        view = [[UIImageView alloc] init];
        view.tag = 11;
        [view setImage:LOADIMAGE(@"浏览次数")];
    ...
    CGSize size = [@(item.scancount).stringValue sizeWithFont:label.font maxWidth:100 maxNumberLines:1];
    label.frame = CGRectMake(0, 10, size.width + 8, 21);
    label.right = App_Frame_Width - 10;
    label.text = @(item.scancount).stringValue;
    view.frame = CGRectMake(label.left - 20, 10, 14, 21);
    return cell;
}
