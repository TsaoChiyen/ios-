...
- (void)viewDidLoad
{
    ...
    self.navigationItem.title = @"申请秘室";

    [self setRightBarButton:@"确定" selector:@selector(rightPressed)];
    [contentArr addObjectsFromArray:@[@"秘室图片", @"秘室标题", @"起始时间", @"结束时间"]];
...
- (void)rightPressed {
    if (!meetTitle) {
        ...
        [super startRequest];
        [self setLoading:YES content:@"正在申请新的秘室..."];
...
- (void)tableView:(UITableView *)sender didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.row == 1) {
        TextEditController * con = [[TextEditController alloc] initWithDel:self type:TextEditTypeDefault title:@"秘室标题" value:meetTitle];
