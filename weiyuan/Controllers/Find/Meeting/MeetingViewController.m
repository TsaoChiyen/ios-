...
- (void)viewDidLoad
{
    ...
    self.navigationItem.title = @"进行中的秘室";
...
- (void)viewDidAppear:(BOOL)animated {
    if (isFirstAppear) {
        [super startRequest];
        [self setLoading:YES content:@"正在获取秘室列表"];
...
- (void)prepareLoadMoreWithPage:(int)page sinceID:(int)sinceID {
    if (self.searchView.width == 0) {
        if (isloadByslime) {
            [self setLoading:YES content:@"正在重置秘室列表"];
        } else {
            [self setLoading:YES content:@"正在获取更多秘室"];
        }
...
- (void)imageTouchViewDidSelected:(ImageTouchView*)sender {
    if ([sender.tag isEqualToString:@"more"]) {
        if (self.searchView.width == 0) {
            MenuView * menuView = [[MenuView alloc] initWithButtonTitles:@[@"进行中的秘室", @"往期秘室", @"我的秘室"] withDelegate:self];
...
- (void)popoverView:(MenuView *)sender didDismissWithButtonIndex:(NSInteger)buttonIndex {
    if (buttonIndex != 3) {
        ...
        [self updateTitleLabel];
        [super startRequest];
        [self setLoading:YES content:@"正在切换秘室列表"];
        [client meetingListWithType:meetType page:currentPage];
    }
}
