...
+ (Address*)AddressWithUUID:(int)uuid {
    Address* result = nil;
    Statement * stmt = [DBConnection statementWithQueryString:[NSString stringWithFormat:@"SELECT * FROM %@ WHERE uuid = ? and currentUser = ?", [self tableName]]];
...
+ (void)deleteWithUID:(NSString*)uid {
    Statement * stmt = [DBConnection statementWithQueryString:[NSString stringWithFormat:@"delete from %@ where withid = ? and currentUser = ?", [self tableName]]];
    int i = 1;
