...
- (void)updateWithJsonDic:(NSDictionary *)dic {
    ...
    if (isInitSuccuss) {
        ...
        _isshop     = [dic getIntValueForKey:@"isshop" defaultValue:0];
        _isfinanc   = [dic getIntValueForKey:@"isfinanc" defaultValue:0];
        _hasShopPass= [dic getIntValueForKey:@"hasShopPass" defaultValue:0];
        _hasExhiPass= [dic getIntValueForKey:@"hasExhiPass" defaultValue:0];
        ...
        _distance   = [dic getStringValueForKey:@"distance" defaultValue:nil];
        NSDictionary * shop = [dic getDictionaryForKey:@"shop"];

        if (shop && shop.count > 0) {
            _shop = [Shop objWithJsonDic:shop];
        }

        NSDictionary * financ = [dic getDictionaryForKey:@"financ"];
        
        if (financ && financ.count > 0) {
            _financ = [Financ objWithJsonDic:financ];
        }
...
- (id)initWithStatement:(Statement *)stmt {
	if (self = [super init]) {
        ...
        _isshop = [stmt getInt32:i++];
        _isfinanc = [stmt getInt32:i++];
        _hasShopPass = [stmt getInt32:i++];
        _hasExhiPass = [stmt getInt32:i++];
...
- (void)insertDB {
    ...
    int i = 1;
    //uid, phone, password, nickname, remark, headsmall, headlarge, sign, gender, province, city, getmsg, isstar, isfriend, verify, sort, isblack, waitforadd, type, isshop, isfinanc, privatePass, fauth1, fauth2, cover, picture1, picture2, picture3
    ...
    [stmt bindInt32:    _isshop     forIndex:i++];
    [stmt bindInt32:    _isfinanc   forIndex:i++];
    [stmt bindInt32:    _hasShopPass forIndex:i++];
    [stmt bindInt32:    _hasExhiPass forIndex:i++];
