...
+ (Session*)sessionWithSubPlat:(SubPlat *)item {
    return [[Session alloc] initWithSubPlat:item];
}
...
- (id)initWithSubPlat:(SubPlat*)item {
    if (self = [super init]) {
        self.typechat = item.usertype==AFRoleService?forChatTypeServiceChat:forChatTypeSubChat;
        self.uid = item.uid;
        self.unreadCount = 0;
        if (!message) {
            ...
            message = [[Message alloc] init];
            message.sendTime = [Globals timeString];
            message.displayImgUrl = user.headsmall;
            message.displayName = user.name;
            message.toname = item.name;
            message.toId = item.uid;
        }
...
+ (id)getSessionWithID:(NSString*)sid {
    Statement * stmt = [DBConnection statementWithQueryString:[NSString stringWithFormat:@"SELECT * FROM %@ WHERE uid = ? AND currentUser = ?", [self tableName]]];
...
- (void)updateSessioninfo {
    Statement * stmt = [DBConnection statementWithQueryString:[NSString stringWithFormat:@"update %@ set unreadCount = ?, sendTime = ?, istop = ? WHERE uid = ? and currentUser = ?", [[self class] tableName]]];
