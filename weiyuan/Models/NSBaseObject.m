    ...
    if (isInitSuccuss) {
        ...
        for(int i = 0; i < propertyCount; i++) {
            ...
            if (strcmp(ivarType, "c") == 0 || strcmp(ivarType, "@\"NSString\"") == 0) {
                ...
                va = [va stringByReplacingOccurrencesOfString :@"\"" withString:@""];
                Class baseClass = NSClassFromString(va);
                if (baseClass && [baseClass isSubclassOfClass:[NSBaseObject class]]) {
                    NSDictionary * dictionary = [dic getDictionaryForKey:key];
                    id value = [baseClass objWithJsonDic:dictionary];
                    [self setValue:value forKey:key];
                } else if (baseClass && [baseClass isSubclassOfClass:[NSArray class]]) {
                    NSArray * arr = [dic getArrayForKey:key];
                    if (arr && arr.count > 0) {
                        Class class = NSClassFromString(key);
                        if (class) {
                            NSMutableArray * muArr = [NSMutableArray array];
                            [arr enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
                                id value = [class objWithJsonDic:obj];
                                [muArr addObject:value];
                            }];
                            [self setValue:muArr forKey:key];
                        }
