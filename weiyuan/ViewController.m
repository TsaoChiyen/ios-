...
- (void)pageHasChanged {
    ...
    if(index ==4) {
        ...
        for(;i<[actionBar.subviews count];i++) {
            UIButton * btn=(UIButton *)[actionBar.subviews objectAtIndex:i];

            if(btn.tag==2) {
                btn.frame= CGRectMake(0, 0, 0, 0);
            } else  {
                btn.frame= CGRectMake(self.view.width - 44*(2-btn.tag), 6, 32, 32);
            }
        }
    } else {
        int i=2;
        ...
        for(;i<[actionBar.subviews count];i++) {
            UIButton * btn=(UIButton *)[actionBar.subviews objectAtIndex:i];
            btn.frame= CGRectMake(self.view.width - 44*(4-btn.tag), 6, 32, 32);
        }
    }
}
