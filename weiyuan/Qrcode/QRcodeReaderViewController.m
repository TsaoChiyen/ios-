...
- (void)decodeZBarView:(id) symbols{
    ...
    if (str && [str isKindOfClass:[NSString class]] && [str hasPrefix:KBSSDKAPIURL]) {
        ...
        if ([self.delegate respondsToSelector:@selector(QRcodeReaderDidFinish:data:)]) {
            [self.delegate performSelector:@selector(QRcodeReaderDidFinish:data:) withObject:self withObject:str];
        }
