...
+ (UIImage*)getImageGoodHeadDefault {
    return [UIImage imageNamed:@"商品默认图片"];
}
...
+ (void)createTableIfNotExists {
    ...
    [ShoppingCart createTableIfNotExists];
}
...
+ (void)smsAction:(NSString *)phone parentView:(BaseViewController*)controller {
    SMSViewController *con = [[SMSViewController alloc] init];
    [con setPhone:phone];
    [controller pushViewController:con];
}
...
{
    Session * session = [Session getSessionWithID:uid];
    if (!session) {
